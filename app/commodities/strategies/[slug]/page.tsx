...
import { COMMODITIES_STRATEGIES } from "../../../lib/services/commoditiesHub";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const strategy = COMMODITIES_STRATEGIES.find((s) => s.id === params.slug);
  ...
  const pageImage = strategy.featuredImage
    ...
    : generateHubImage("commodities", "strategy");

  return buildPageMetadata({
    title: `${strategy.name} | Commodities Trading Strategies`,
    description: strategy.description,
    canonical: `https://pricelot.com/commodities/strategies/${params.slug}`,
...
export async function generateStaticParams() {
  return COMMODITIES_STRATEGIES.map((strategy) => ({ slug: strategy.id }));
}

export default async function CommoditiesStrategyPage({ params }: Props) {
  const strategy = COMMODITIES_STRATEGIES.find((s) => s.id === params.slug);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-950">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <nav className="mb-8 flex text-sm text-slate-400">
          ...
          <a href="/commodities?tab=strategies" className="hover:text-emerald-300">Strategies</a>
          <span className="mx-2">/</span>
          <span className="text-emerald-300">{strategy.name}</span>
        </nav>

        <div className="mb-8">
          <div className="mb-3 flex gap-2">
            <div className="rounded-full bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-300">{strategy.category}</div>
          ...
          <ContentMediaHero
            ...
            fallbackImage={generateHubImage("commodities", "strategy")}
            title={strategy.name}
        ...
        <div className="mb-12">
          <h2 className="mb-6 text-2xl font-bold text-emerald-300">Strategy Rules</h2>
          <div className="space-y-4">
            {strategy.rules.map((rule, idx) => (
              <div key={idx} className="rounded-lg border border-emerald-500/10 bg-slate-900/30 p-6">
                <div className="flex items-start gap-4">
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-300 font-bold text-sm">{idx + 1}</div>
                  <div>
