...
import { COMMODITIES_ARTICLES } from "../../../lib/services/commoditiesHub";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const article = COMMODITIES_ARTICLES.find((a) => a.slug === params.slug);
  ...
  return {
    title: `${article.title} | Commodities Trading Hub`,
...
export async function generateStaticParams() {
  return COMMODITIES_ARTICLES.map((article) => ({ slug: article.slug }));
}

export default async function CommoditiesArticlePage({ params }: Props) {
  const article = COMMODITIES_ARTICLES.find((a) => a.slug === params.slug);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-950">
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            image: "https://pricelot.com/commodities-banner.jpg",
          }),
        }}
      />
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <nav className="mb-8 flex text-sm text-slate-400">
          ...
          <a href="/commodities?tab=articles" className="hover:text-emerald-300">Articles</a>
          <span className="mx-2">/</span>
          <span className="text-emerald-300">{article.title}</span>
        </nav>

        <div className="mb-8">
          <div className="mb-3 inline-block rounded-full bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-300">{article.category}</div>
