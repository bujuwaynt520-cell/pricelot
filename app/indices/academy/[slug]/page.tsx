...
import { INDICES_LESSONS } from "../../../lib/services/indicesHub";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const lesson = INDICES_LESSONS.find((l) => l.id === slug);
  ...
  const pageImage = lesson.featuredImage
    ...
    : generateHubImage("indices", "lesson");

  return buildPageMetadata({
    title: `${lesson.title} | Indices Trading Academy`,
    description: lesson.summary,
    canonical: `https://pricelot.com/indices/academy/${slug}`,
...
export async function generateStaticParams() {
  return INDICES_LESSONS.map((lesson) => ({ slug: lesson.id }));
}

export default async function IndicesLessonPage({ params }: Props) {
  const { slug } = await params;
  const lesson = INDICES_LESSONS.find((l) => l.id === slug);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-sky-950 to-slate-950">
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        <nav className="mb-8 flex text-sm text-slate-400">
          ...
          <a href="/indices?tab=lessons" className="hover:text-sky-300">Academy</a>
          <span className="mx-2">/</span>
          <span className="text-sky-300">{lesson.title}</span>
        </nav>

        <div className="mb-8">
          <div className="mb-3 flex gap-2">
            <div className="rounded-full bg-sky-500/20 px-4 py-2 text-sm font-semibold text-sky-300">{lesson.category}</div>
            <div className="rounded-full bg-cyan-500/20 px-4 py-2 text-sm font-semibold text-cyan-300">{lesson.difficulty}</div>
          </div>
          ...
          <ContentMediaHero
            ...
            fallbackImage={generateHubImage("indices", "lesson")}
            title={lesson.title}
          ...
          {lesson.content.map((paragraph, idx) => (
            <div key={idx} className="rounded-lg border border-sky-500/10 bg-slate-900/30 p-6">
              <div className="flex items-start gap-4">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-sky-500/20 text-sky-300 font-bold">{idx + 1}</div>
        ...
        <div className="mb-12 rounded-lg border border-sky-500/20 bg-sky-500/5 p-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h3 className="mb-2 text-lg font-bold text-sky-300">Ready to apply what you learned?</h3>
              <p className="text-slate-300">Explore our indices strategies and practice with structured market entries.</p>
            </div>
            <SaveLessonButton
              lessonId={slug}
              title={lesson.title}
