...
import { CRYPTO_ARTICLES, CRYPTO_LESSONS, CRYPTO_STRATEGIES, CRYPTO_GLOSSARY, CryptoCategory } from "../lib/services/cryptoHub";

export default function CryptoHubClient() {
  ...
  const filteredArticles = useMemo(() => {
    return CRYPTO_ARTICLES.filter((article) => {
      const matchesSearch =
  ...
  const filteredLessons = useMemo(() => {
    return CRYPTO_LESSONS.filter((lesson) => {
      const matchesSearch =
  ...
  const filteredStrategies = useMemo(() => {
    return CRYPTO_STRATEGIES.filter((strategy) => {
      const matchesSearch =
  ...
  const filteredGlossary = useMemo(() => {
    return CRYPTO_GLOSSARY.filter((term) => {
      const matchesSearch =
      ...
      <div className="mx-auto max-w-6xl">
        <div id="search" className="mb-12">
          <div className="mb-6 flex flex-col gap-4">
            <input
              type="text"
              placeholder="Search crypto articles, lessons, strategies, glossary..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="rounded-lg border border-cyan-500/30 bg-slate-900/50 px-4 py-3 text-white placeholder-slate-500 transition focus:border-cyan-400 focus:outline-none"
            />
          </div>
            ...
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
                selectedCategory === "all"
                  ...
                  : "border border-cyan-500/30 text-cyan-300 hover:border-cyan-400"
              }`}
            >
              All Categories
            </button>
        ...
        <div className="mb-8 flex border-b border-cyan-500/30">
          {[
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {filteredArticles.map((article) => (
              <a
                ...
                className="group rounded-lg border border-cyan-500/20 bg-slate-900/50 p-6 transition hover:border-cyan-400 hover:bg-slate-800/50"
              >
                <div className="mb-2 text-xs font-semibold text-cyan-400">{article.category}</div>
                <h3 className="mb-3 text-lg font-bold text-white group-hover:text-cyan-300">{article.title}</h3>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {filteredLessons.map((lesson) => (
              ...
              >
                ...
                <h3 className="mb-3 text-lg font-bold text-white group-hover:text-cyan-300">{lesson.title}</h3>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {filteredStrategies.map((strategy) => (
              ...
              >
                ...
                <h3 className="mb-3 text-lg font-bold text-white group-hover:text-cyan-300">{strategy.name}</h3>
        ...
        {activeTab === "glossary" && (
          <div className="space-y-4">
            {filteredGlossary.map((term) => (
              <div key={term.id} className="rounded-lg border border-cyan-500/20 bg-slate-900/50 p-6">
                <div className="mb-2 flex items-center justify-between">
                  <h3 className="text-lg font-bold text-cyan-300">{term.term}</h3>
                  <span className="text-xs font-semibold text-cyan-400">{term.category}</span>
                </div>
        ...
        {searchTerm && filteredArticles.length === 0 && filteredLessons.length === 0 && filteredStrategies.length === 0 && filteredGlossary.length === 0 && (
          <div className="rounded-lg border border-cyan-500/20 bg-slate-900/50 p-12 text-center">
